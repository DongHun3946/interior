...
class R2StorageTest(unittest.TestCase):
    ...
    def test_prefixes_object_key_with_configured_storage_prefix(self):
        client = FakeS3Client()
        configured = {
            "storage_backend": "r2",
            "storage_prefix": "/interior-v1/",
            "r2_account_id": "account-id",
            "r2_access_key_id": "access-key",
            "r2_secret_access_key": "secret-key",
            "r2_bucket_name": "jeil-interior",
        ...
        with patch.multiple(storage.settings, **configured), patch.object(storage, "_r2_client", return_value=client):
            key, url, _ = storage.save_upload("project-id", image_upload())

        self.assertTrue(key.startswith("interior-v1/project-id/"))
        self.assertEqual(client.upload["key"], key)
        self.assertEqual(url, f"https://images.example.com/{key}")
